...
import { KBGraph } from "../../sdk/models";
import { KBGraphApi, KB_EntityApi, KB_RelationshipApi } from "../../sdk/services";
...
export class GraphComponent implements OnInit {
  ...
  constructor(
    ...
    private kbgraphApi: KBGraphApi,
    private kbentityApi: KB_EntityApi,
    private kbrelationshipApi: KB_RelationshipApi,
  ) {
    ...
    this.nodesList = [
      ...
      { name: "患者指导", type: "patient_guidance", color: 4 },
      { name: "观察项", type: "observation_item", color: 5 },
      { name: "观察项标准", type: "item_criterion", color: 6 },
      { name: "检验", type: "lab_test", color: 7 },
      { name: "检查", type: "medical_examination", color: 8 },
      { name: "诊断", type: "diagnosis", color: 9 },
      { name: "器件", type: "device", color: 10 },
      { name: "人体部位", type: "body_part", color: 11 },
      { name: "其它因素", type: "context", color: 12 },
      { name: "时间", type: "manifestation", color: 13 },
      { name: "描述", type: "descriptor", color: 14 }
    ]
  ...
  private resize() {
    let viewWidth = this._getWidth(this.d3Graph),
      ...
      panWidth = this._calPanWidth(this.d3Graph, 4),
      panHeight = this._calPanHeight(this.d3Graph, 4),
      thisGraph = this;
  ...
  private updateGraph() {
    ...
    this._paths = this._pathsG.selectAll("path.link").data(this.linksGroup);
    this._pathsText = this._pathsG.selectAll("text").data(this.linksGroup);
    ...
    this._paths.enter().append("path")
      ...
      .attr("d", function (d, index) {
        ...
        sourceNode = thisGraph.nodes.filter(node => node.id == d.source_id)[0];
        targetNode = thisGraph.nodes.filter(node => node.id == d.destination_id)[0];
      ...
      .attr("", function (d, index) {
        if (d.count >= 1 && _.pluck(d.links, "name").length > 0) {
          let textElement = $("#text_" + d.id);
          let currentScale = thisGraph.getTransform(thisGraph._svgG).scale;
          let halfPosition = this.getPointAtLength(this.getTotalLength() / 2);
          if (textElement[0]) {
            let x = halfPosition.x - (textElement.width() + thisGraph._CONST.nodeWidth / 2) * currentScale / 2;
            textElement.attr({
              x: halfPosition.x,
              y: halfPosition.y - 5
            })
            if (d.count == 1) {
              textElement.html(d.links[0].name)
            } else {
              textElement.html(d.count)
            }
          } else {
            let textTag = thisGraph.createSVGNode("text");
            if (d.count == 1) {
              $(textTag).html(d.links[0].name)
            } else {
              $(textTag).html(d.count)
            }
      ...
      .on("click", function (data) {
        thisGraph.linkEdit = true;
        thisGraph.formType = undefined;
        thisGraph.singleData = data;
        thisGraph.snav.open();
      })


    // this._pathsText.enter().append("text")
    //   .attr("x", 8)
    //   .attr("dy", 28)
    //   .append("textPath")
    //   .attr("xlink:href", function (d, i) { return "#" + d.id; })
    //   .merge(this._pathsText)
  ...
  private frameInit() {
    ...
    this._minimap.call(this._CONST.miniZoom);
  }
        ...
        if (thisGraph._state.dragDraw) {
          ...
          d3.selectAll("path.link")
            .filter(function (link) {
              ...
              if (link) {
                let sourceNode = thisGraph.nodes.filter(node => node.id == link.source_id)[0],
                  targetNode = thisGraph.nodes.filter(node => node.id == link.destination_id)[0];
            ...
            .attr("d", function (current) {
              ...
              //   })
              // }
              // if (current.source_id === current.destination_id) {
              //   var node = underscore.find(thisGraph.nodes, function (node) {
              //     return node.id === current.source_id;
              //   })
              //   var startPoint = [
              ...
              let sourceNode, targetNode, sourceX,
                sourceY, targetX, targetY;
              sourceNode = thisGraph.nodes.filter(node => node.id == current.source_id)[0];
              targetNode = thisGraph.nodes.filter(node => node.id == current.destination_id)[0];
  ...
  private nodeMouseUp(d3node, d) {
    ...
    if (mouseUpNode && this._state.dragDraw) {
      ...
      let newLink = {
        id: new Date().getTime(),
        source_id: mouseDownNode.id,
        destination_id: mouseUpNode.id,
        type: "Relationship"
      }

      let tmpConflickLink = this.links.filter(link => {
        return link.destination_id === newLink.source_id && link.source_id === newLink.destination_id
      })
  ...
  private toolbarEdit(selection, data) {
    ...
    this.formType = this.singleData.entity_type;
  ...
  private toolbarDelete(selection, data) {
    let i = this.nodes.findIndex(node => node.id === data.id), thisGraph = this,
      toSpliceLinks = this.links.filter(link => link.destination_id === data.id || link.source_id === data.id);
  ...
  brushEnd = function () {
    if (d3.event.selection != null) {
      d3.select("g.brush").call(d3.event.destination_id.move, null);
    }
  }
  ...
  private groupLink() {
    ...
    this.links.forEach(link => {
      if (link.source_id && link.destination_id) {
        let sourceNode: any = this.nodes.filter(node => node.id === link.source_id)[0];
        let targetNode: any = this.nodes.filter(node => node.id === link.destination_id)[0];
        if (!sourceNode || !targetNode) return;

        let tmpID = sourceNode.id + "-" + targetNode.id;
        let tmpIndex = this.linksGroup.findIndex(item => item.id === tmpID);
        if (tmpIndex < 0) {
          this.linksGroup.push({
            id: tmpID,
          ...
          this.linksGroup[tmpIndex].links.push(link);
          this.linksGroup[tmpIndex].count += 1;
        }
      }
    })
  ...
  save() {
    ...
    if (this.linkEdit) {
      let links = this.singleData.links;
      links.forEach((link, index) => {
        this.kbrelationshipApi.updateAttributes(link.id, link)
          .subscribe(result => {
      ...
      this.kbentityApi.updateAttributes(this.singleData.id, this.singleData).subscribe(ret => {
        console.log(ret);
        this.updateGraph();
        this.snav.close();
